...
class CreateLotsPage extends StatefulWidget {
  const CreateLotsPage({Key? key, required this.template, required this.roundIndex, required this.lots}) : super(key: key);
...
class _CreateLotsPageState extends State<CreateLotsPage> {
  ...
  void initState() {
    super.initState();
    if(widget.template.data.betParams!=null && widget.template.data.betParams!.startSum!=null) {
      defaultStartSum = widget.template.data.betParams!.startSum!;
    }
    if(widget.template.data.betParams!=null && widget.template.data.betParams!.limitSum!=null) {
      defaultLimitSum = widget.template.data.betParams!.limitSum!;
    }

    if(widget.template.data.rounds[widget.roundIndex].betParams!=null && widget.template.data.rounds[widget.roundIndex].betParams!.startSum!=null) {
      defaultStartSum = widget.template.data.rounds[widget.roundIndex].betParams!.startSum!;
    }
    if(widget.template.data.rounds[widget.roundIndex].betParams!=null && widget.template.data.rounds[widget.roundIndex].betParams!.limitSum!=null) {
      defaultLimitSum = widget.template.data.rounds[widget.roundIndex].betParams!.limitSum!;
    }
              ...
              for(var name in nameControl) {
                if(name.text.isEmpty) {
                  ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                          content: Text("Some names are empty")));
                  return;
                }
              }
          ...
          FloatingActionButton(
            onPressed: () {
              setState(() {
                ...
                widget.lots.add(LotRequest(duration: null, name: "", description: "", betParams: null));
              ...
              children: [
                ...
                Align(
                  ...
                  child: RawAutocomplete<String>(
                    ...
                    optionsBuilder: (TextEditingValue textEditingValue) {
                      return widget.template.data.lotNames.where((String option) {
                ...
                Align(
                  ...
                  child: RawAutocomplete<String>(
                    ...
                    optionsBuilder: (TextEditingValue textEditingValue) {
                      return widget.template.data.lotDescriptions.where((String option) {
                        return option.contains(textEditingValue.text.toLowerCase());
                      }).toList();
                    },
                    optionsViewBuilder: (
                    ...
                    Expanded(child:TextFormField(
                      ...
                      onChanged: (text) {
                        if(widget.lots[i].betParams==null) {
                          widget.lots[i].betParams = BetParams(startSum: int.parse(text), limitSum: null, minBetStep: null);
                          return;
                        }
                        widget.lots[i].betParams!.startSum = int.parse(text);
                      },
                    ),
                    ),
                    Expanded(child:TextFormField(
                      onChanged: (text) {
                        if(widget.lots[i].betParams==null) {
                          widget.lots[i].betParams = BetParams(startSum: null, limitSum: int.parse(text), minBetStep: null);
                          return;
                        }
                        widget.lots[i].betParams!.limitSum = int.parse(text);
                      },
                      inputFormatters: [FilteringTextInputFormatter.digitsOnly],
                      keyboardType: TextInputType.number,
